        ...
        try {
            ...
            if (content) {
                ...
                for (let line of lines) {
                    ...
                    if (line.indexOf('!>') !== -1) {
                        frame = 'warn';
                        msg += `行数 #${number + 1}：${line.slice(line.indexOf('!>') + 2)}<br />`;
                ...
                msg = `成功转换 ${lines.length} 行代码${lines.length === 1 ? '' : 's'} (耗时 ${(timeDelta / 1000).toFixed(3)} 秒)<br />${msg}<a href="#" onclick="javascript:copy();">复制到剪切板</a>`;
            }
        }
        catch (ex) {
            frame = 'error';
            msg = `转换时发生错误 <br />行数 #${number + 1} ${ex}`;
            result = '';
        }
        finally {
            $('#info').hide();
...
class Blocks {
    ...
    static get1_12NominalIDFrom1_12StringIDWithMetadata(id, metadata) {
        ...
        if (arr) {
            ...
            throw `未知 1.12 物品 ID: '${id}:${metadata}'`;
        }
    }
    ...
    static get1_13NominalIDFrom1_12NumericID(id, metadata) {
        ...
        if (nominalID1_12) {
            ...
            throw `未知 1.12 数字 ID: '${id}:${metadata}'`;
        }
    }
...
class SpuScript {
                    ...
                    switch (name) {
                        case 'addAdvToEntity': {
                            ...
                            if (params.length === 1) {
                                ...
                                throw `意外的参数计数: ${params.length} 关于 ${name} 在 ${arg}.`;
                            }
                            source = sel.get1_13();
                            break;
                        }
    ...
    getAst(arg) {
        ...
        if (char === '%') {
            ...
            throw `未知的 Token: ${char} 在 ${arg}. 这应该是 '%".`;
        }
        while (char && char !== '$') {
            id += char;
            char = charReader.next();
    ...
    parseCompound(tokens, pos) {
        ...
        for (; pos < tokens.length; pos++) {
            ...
            if (expectedTypes.indexOf(token.type) !== -1) {
                ...
                throw `发生错误 '${expectedTypes}' 但是得到了 '${token.type}' 在 '${pos}'.`;
            }
        }
        throw '多重解析错误';
    }
    parseList(tokens, pos) {
        ...
        for (; pos < tokens.length; pos++) {
            ...
            if (expectedTypes.indexOf(token.type) !== -1) {
                ...
                throw `发生错误 '${expectedTypes}' 但是得到 '${token.type}' 在 '${pos}'.`;
            }
        }
        throw '多重解析错误';
    }
                ...
                switch (token.type) {
                    ...
                    case 'Thing':
                        ...
                        if ((val = this.parseByte(token)) !== null) {
                            ...
                            throw `得到 Token 在 '${pos}' 但是在解析数组的时候，其类型不是比特`;
                        }
                        break;
                    case 'Comma':
                        expectedTypes = ['EndListOrArray', 'Thing'];
                ...
                switch (token.type) {
                    ...
                    case 'Thing':
                        ...
                        if ((val = this.parseLong(token)) !== null) {
                            ...
                            throw `得到 Token 在 '${pos}' 但是在解析数组的时候，其类型不是Long`;
                        }
                        break;
                    case 'Comma':
                        expectedTypes = ['EndListOrArray', 'Thing'];
...
class Selector {
    ...
    parseAdvancements1_13(val) {
        ...
        let map;
        if (char !== '{') {
            throw `成就应该以 '{' 开头, 但是得到 '${char}' 在 '${val}'`;
        }
        char = charReader.next();
        if (char === '}') {
            return;
